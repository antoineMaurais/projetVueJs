...
export default {
  ...
  methods: {
    getOccurenceElement(key) {
      ...
      let tableauElementsUnique = [...new Set(tousLesElements)]; //enlève les doublons
  ...
  computed: {
    ...
    statistiqueNbClassesDiff() {
      ...
      for (const vFor of this.data) {
        if (vFor[3].length != 0) {
          // parcours des classes présentes dans la partie classe de chaque sort = parcours sous-tableau
          for (const classe of vFor[4]) {
            //verification si déjà présent
      ...
      return countArrayClasse.length;
    },
  },
};
</script>
